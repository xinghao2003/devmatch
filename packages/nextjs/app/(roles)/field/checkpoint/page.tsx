...
const FieldCheckpointPage: React.FC = () => {
  const { user } = useAuth();
  ...
  const onSubmit = async (evt: React.FormEvent) => {
    evt.preventDefault();
    if (user?.role !== "field") {
      toast.error("Only the FIELD wallet can add checkpoints");
      return;
    }
    ...
    try {
      setSubmitting(true);
      await writeContractAsync({ functionName: "addCheckpoint", args: [shipmentIdBigInt, location.trim()] });
    ...
    <div className="max-w-xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="glass-card p-6">
          <h1 className="text-2xl font-bold text-white mb-1">Add Checkpoint</h1>
          <p className="text-white/70 mb-6">Log the latest location for a shipment.</p>
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            >
              {submitting || isPending || isMining ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
              <span>{submitting || isPending || isMining ? "Submitting..." : "Submit Checkpoint"}</span>
            </button>
          </form>
        </div>
      </motion.div>
...
export default FieldCheckpointPage;
